...
import { Search, MapPin, Sparkles, Filter, Check, X, GraduationCap, Home, Users, Utensils } from 'lucide-react';
import { PROPERTY_TYPES, BHK_OPTIONS, JAMMU_COLLEGES } from '../data/citiesAndLocalities';
...
export default function HeroSearch({ 
  ...
  // Deterministic AI Natural Language parser helper for non-tech-savvy users
  const handleAiSearchParse = (e) => {
    ...
    // Check PG vs Rent vs Buy
    if (query.includes('pg') || query.includes('hostel') || query.includes('sharing') || query.includes('student')) {
      setActiveTab('pg');
      if (query.includes('boys')) setPgGender && setPgGender('Boys');
      if (query.includes('girls') || query.includes('female')) setPgGender && setPgGender('Girls');
    } else if (query.includes('rent') || query.includes('lease')) {
      setActiveTab('rent');
    ...
    // Check College proximity matching
    const matchedCollege = JAMMU_COLLEGES.find(col => 
      query.includes(col.id) || query.includes(col.name.toLowerCase()) || query.includes('miet') || query.includes('jammu university')
    );
    if (matchedCollege && setSelectedCollege) {
      setSelectedCollege(matchedCollege.id);
    }
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-2xl sm:text-4xl font-extrabold tracking-tight text-white mb-2">
            Find Your Ideal {activeTab === 'pg' ? 'Student PG & Hostel' : 'Home'} in <span className="text-amber-400 border-b-2 border-amber-400 pb-0.5">{selectedCity.name}</span>
          </h1>
        ...
        <div className="bg-white text-slate-900 rounded-b-xl rounded-tr-xl sm:rounded-tr-none p-5 shadow-lg border border-slate-200">
          ...
          {/* PG Discovery Specific Form Controls */}
          {activeTab === 'pg' ? (
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
                {/* Search Input (Locality / College Name) */}
                <div className="md:col-span-5 relative">
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">
                    Locality / College Campus
                  </label>
                  <div className="relative">
                    ...
                    <input 
                      ...
                      placeholder="e.g. Near MIET, Kot Bhalwal, Channi..."
                      className="w-full bg-purple-50/50 border border-purple-200 rounded-md pl-9 pr-3 py-2 text-sm font-medium text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-600 focus:bg-white"
                    />
                  </div>
                </div>

                {/* College Proximity Selector */}
                <div className="md:col-span-4">
                  <label className="block text-xs font-bold text-purple-900 uppercase tracking-wider mb-1 flex items-center gap-1">
                    <GraduationCap className="w-3.5 h-3.5 text-purple-700" />
                    Target College (Distance Calculator)
                  </label>
                  <select 
                    value={selectedCollege || ''}
                    onChange={(e) => setSelectedCollege && setSelectedCollege(e.target.value)}
                    className="w-full bg-purple-50 border border-purple-200 rounded-md px-3 py-2 text-sm font-bold text-purple-950 focus:outline-none focus:ring-2 focus:ring-purple-600"
                  >
                    <option value="">All Colleges in {selectedCity.name}</option>
                    {JAMMU_COLLEGES.map(col => (
                      <option key={col.id} value={col.id}>{col.name} ({col.locality})</option>
                    ))}
                  </select>
                </div>

                {/* Monthly Rent Budget Dropdown */}
                <div className="md:col-span-3">
                  ...
                  <select 
                    ...
                    className="w-full bg-slate-50 border border-slate-300 rounded-md px-3 py-2 text-sm font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-purple-600"
                  >
                    ...
                    <option value="10k-plus">Above ₹10,000 / mo</option>
                  </select>
                </div>
              </div>

              {/* PG Filters Row: Gender, Room Type, Food */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-3 border-t border-slate-100 text-xs">
                {/* Gender Category Selector */}
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Gender / Category</label>
                  <div className="flex gap-1">
                    {['All', 'Boys', 'Girls', 'Co-living'].map(genderOpt => (
                      <button
                        key={genderOpt}
                        type="button"
                        onClick={() => setPgGender && setPgGender(genderOpt)}
                        className={`flex-1 py-1.5 rounded font-bold transition-all text-center text-[11px] ${
                          pgGender === genderOpt 
                ...
                {/* Room Sharing Type */}
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Room Sharing Type</label>
                  <div className="flex gap-1">
                    {['All', 'Single', '2 Sharing', '3 Sharing'].map(roomOpt => (
                      <button
                        key={roomOpt}
                        type="button"
                        onClick={() => setPgRoomType && setPgRoomType(roomOpt)}
                        className={`flex-1 py-1.5 rounded font-bold transition-all text-center text-[11px] ${
                          pgRoomType === roomOpt 
                            ? 'bg-purple-900 text-white shadow-xs' 
                            : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Mess & Food Service</label>
                  <div className="flex gap-1">
                    {['All', 'Food Included', 'No Food'].map(foodOpt => (
                      <button
                        key={foodOpt}
                        type="button"
                        onClick={() => setFoodIncluded && setFoodIncluded(foodOpt)}
                        className={`flex-1 py-1.5 rounded font-bold transition-all text-center text-[11px] ${
                          foodIncluded === foodOpt 
                            ? 'bg-purple-900 text-white shadow-xs' 
                            : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                        }`}
                      >
            ...
            /* Standard Property Search Controls */
            <div>
              ...
              {/* BHK Selection & Popular Localities */}
          ...
          {/* Natural Language AI Assistant Search Bar */}
          <div className="mt-4 pt-4 border-t border-slate-100 bg-slate-50 p-3 rounded-lg border border-slate-200">
            <form onSubmit={handleAiSearchParse} className="flex items-center gap-2">
              ...
              <input 
                ...
                placeholder={activeTab === 'pg' ? `e.g. "Boys PG near MIET Jammu under 8000"` : `e.g. "3 BHK ready to move in Gandhi Nagar under 1 Cr"`}
